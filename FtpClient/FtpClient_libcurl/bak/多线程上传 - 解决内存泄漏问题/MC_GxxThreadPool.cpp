...
bool MC_GxxThreadPool::OnLoad(int vMinThreads, int vMaxThreads)
{
	...
	//任务获取线程
	if(false == m_ThreadGetTask.Start(GetTaskThreadFunc, this))
	{
...
bool MC_GxxThreadPool::OnExit(void)
{
	...
	if(true == m_ThreadGetTask.IsRunning())
	{
		m_ThreadGetTask.Stop();
		m_ThreadGetTask.Join();
	}
...
//---------------------------------------------------------------------------
void GS_CALLBACK MC_GxxThreadPool::GetTaskThreadFunc(GSThread& vThread, void* pUserData)
{
	MC_GxxThreadPool* tKernelWork = (MC_GxxThreadPool*)pUserData;
	if(NULL == tKernelWork)
		return;

	/*if(true == vThread.TestExit())
	{
		break;
	}*/
