...
//---------------------------------------------------------------------------
bool MC_FtpClient::UploadFile(SOCKET vCtrlSocket, const char* vLoaclFilePath, const char* vRemoteFilePath, bool vResume, ProgressCallback vCB)
{
	if(false == m_Available)						return false;
	...
	//远程传输开始
	char tCmd[FTP_CMD_SIZE] = {0};
	if(tLocalFileSize > tRemoteFileSize && tRemoteFileSize > 0)
	{
		sprintf_s(tCmd, sizeof(tCmd), "APPE %s\r\n", tRemoteFile);
	}
	else
	{
		tRemoteFileSize = 0;	//服务器文件比本地文件大，是否覆盖？
...
bool MC_FtpClient::CheckResult(SOCKET vCtrlSocket, const char vResult, const char* vCmd, char* vResponse)
{
	...
	{
		char* tTrimCmd = trim(vCmd);
		LOG_ERROR("recv[1] response fail, cmd: %s, errno: %d", tTrimCmd, WSAGetLastError());
		delete[] tTrimCmd;
		return false;
	}
	...
	if(vResult != tResponse[0])
	{
		...
		if(NULL != strstr(vCmd, "SIZE") && NULL != strstr(tResponse, "cannot find the"))	//不输出日志
			return false;

		char* tTrimCmd = trim(vCmd);
		char* tTrimRes = trim(tResponse);
		LOG_ERROR("undesired response, cmd: %s, response: %s", tTrimCmd, tTrimRes);
		delete[] tTrimCmd;
		delete[] tTrimRes;
		return false;
	}
...
bool MC_FtpClient::SendCmd(SOCKET vCtrlSocket, const char* vCmd)
{
	...
	{
		char* tTrimCmd = trim(vCmd);
		LOG_ERROR("send cmd fail, cmd: %s, errno: %d", tTrimCmd, WSAGetLastError());
		delete[] tTrimCmd;
		return false; 
	}
